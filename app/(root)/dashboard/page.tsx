...
export default function TeacherDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const lessonsData = lessonsSnapshot.docs.map(
        (doc) =>
          ({
            id: doc.id,
            ...doc.data(),
  ...
  const handleSlotToggle = async (slot: TimeSlot) => {
    ...
    const isSlotAvailable = availableSlots.some(
      (s) =>
        isSameDay(s.start, slot.start) &&
        s.start.getHours() === slot.start.getHours(),
    );

    try {
      let updatedSlots: TimeSlot[];
      if (isSlotAvailable) {
        updatedSlots = availableSlots.filter(
          (s) =>
            !(
              isSameDay(s.start, slot.start) &&
              s.start.getHours() === slot.start.getHours()
...
const BookingSlotsCalendar: React.FC<BookingSlotsCalendarProps> = ({
  availableSlots,
  currentWeek,
  setCurrentWeek,
  handleSlotToggle,
  ...
  <Card>
    ...
    <CardContent>
      ...
      <div className="grid grid-cols-7 gap-2">
        {Array.from({ length: 7 }, (_, i) => addDays(currentWeek, i)).map(
          (day, index) => (

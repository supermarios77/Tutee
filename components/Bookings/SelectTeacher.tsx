...
export const SelectTeacher: React.FC<SelectTeacherProps> = ({ onSelectTeacher, selectedPlan, teachers }) => {
  const isTeacherAvailable = (teacher: Teacher) => {
    if (!teacher.availability) return false;
    const today = new Date().toLocaleString('en-us', {weekday: 'long'});
    return teacher.availability[today] && teacher.availability[today].length > 0;
  };
      ...
      {teachers.map((teacher) => (
        <Card key={teacher.id} className="hover:shadow-lg transition-shadow duration-300">
          <CardHeader>
            ...
            <CardTitle className="text-center mt-2">{teacher.name || 'Unknown Teacher'}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-gray-600 mb-2">{teacher.bio || 'No bio available'}</p>
            <p className="text-sm font-semibold mb-2">
              Specializations: {teacher.specializations ? teacher.specializations.join(', ') : 'Not specified'}
            </p>
            <Button 
